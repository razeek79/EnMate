...
export default function InteractiveCalculator() {
  ...
  const [costMin, setComputedMin] = useState(0);
  const [costMax, setComputedMax] = useState(0);
  const [timelineMin, setTimelineMin] = useState(2);
  const [timelineMax, setTimelineMax] = useState(4);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      {/* 🧼 CLEANED UP DUPLICATE CURSOR NODES & UNUSED GRID IMPORTS FROM FORM SURFACE 🧼 */}
      ...
      <aside className="lg:col-span-5 bg-[#040208]/90 border border-white/5 rounded-3xl p-6 md:p-8 lg:sticky lg:top-24 shadow-2xl space-y-6 text-left">
        ...
        <div className="border-t border-white/5 pt-5 space-y-4">
          ...
          {/* 💎 FIX: Restored button overlay classes to run moving neon glows and slide reflections perfectly 💎 */}
          <button 
            type="button" 
