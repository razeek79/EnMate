...
export default function HomePage() {
  ...
  // ─── MASTER ULTRA-PREMIUM DUAL-RING CUSTOM CURSOR INTERACTOR ───
  useEffect(() => {
    const dot = document.getElementById('cursor-dot');
    const ring = document.getElementById('cursor-ring');
    if (!dot || !ring) return;

    // Track real mouse vectors
    let mouseX = 0;
    let mouseY = 0;
    
    // Trailing ring floating coordinate metrics
    let ringX = 0;
    let ringY = 0;
    ...
    const onMouseMove = (e) => {
      ...
      // Update the solid instant center dot coordinate manually
      dot.style.left = `${mouseX}px`;
      dot.style.top = `${mouseY}px`;

      // Spotlight calculations inside grid blocks
    ...
    // Physics Loop calculation for smooth trailing ring friction mapping
    const tick = () => {
      // 0.12 creates an elegant, slightly delayed smooth ease behind the center dot
    ...
    // Add Hover expansion classes on clickable items
    ...
    // Observe dynamic elements and map triggers
    addHoverListeners();
        ...
        <div className="min-h-screen w-full bg-gradient-to-b from-[#05030a]/45 via-[#05030a]/30 to-[#0b0410]/45">
          ...
          <main>
              ...
              <div className="container">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center reveal-on-scroll">
                  ...
                  <div className="lg:col-span-5 grid grid-cols-2 gap-4">
                    <div className="p-4 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl text-center transition-all duration-300 hover:border-[var(--accent-soft)] hover:shadow-[0_0_20px_rgba(207,4,102,0.15)]">
                      <span className="text-[var(--accent-soft)] text-sm font-bold block">4+</span>
                      <span className="text-xs text-[var(--text-muted)]">Core Disciplines</span>
                    </div>
                    <div className="p-4 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl text-center transition-all duration-300 hover:border-[var(--accent-soft)] hover:shadow-[0_0_20px_rgba(207,4,102,0.15)]">
                      <span className="text-[var(--accent-soft)] text-sm font-bold block">100%</span>
                      <span className="text-xs text-[var(--text-muted)]">Custom Code</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <section id="process" className="process-advanced">
              <div className="container process-wrapper reveal-on-scroll">
